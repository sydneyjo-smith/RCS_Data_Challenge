...
plot_2_data_filtered <- plot_2_data %>%
  filter(audit == 'NNHLA' &
           trust_code == 'RJ1')

# filtered_data <- data %>%
#   filter(trust_code == 'RJ1')


# Reshape the data to long format
data_long <- plot_2_data_filtered %>%
...
performance_plot <- ggplot(data_long, aes(x = quarter_year, y = percentage, color = category, group = category, text=text)) +
  geom_line(data = filter(data_long, category == "Trust moving average"), size = 1, ) +  # Lines for ca_mav and na_mav
  geom_line(data = filter(data_long, category != "Trust moving average"), size = 1, linetype="dotted") +  # Lines for ca_mav and na_mav
  geom_point(data = filter(data_long, category == "Trust moving average"),
  ...
  geom_point(data = filter(data_long, category == "Trust moving average" & is.na(denominator)),
