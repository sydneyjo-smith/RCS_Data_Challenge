...
### COMPLETENESS OF CANCERS AND METRICS (FOR BOTH DATA QUALITY & PERFORMANCE INDICATOR) BY TRUST 
...
for (trust in unique_data_quality_trusts) {
  ...
  summary_data_quality <- rbind(summary_data_quality, audit_metrics) %>% arrange(trust_name)
  }
...
for (trust in unique_Indicators_trusts) {
  ...
  # Append the new row to the summary data frame
  summary_indicators_trust <- rbind(summary_indicators_trust, audit_metrics) %>% arrange(trust_name)
...
### COMPLETENESS OF QUARTERLY REPORTS BY TRUST
...
duplicates_data_quality <- data_quality %>%
  group_by(audit, metric_name, trust_name, quarter_year) %>%
  filter(n() > 1) %>%
  ungroup()
...
duplicates_performance_indicator <- Indicators_trust %>%
  group_by(audit, metric_name, trust_name, quarter_year) %>%
  filter(n() > 1) %>%
  arrange(audit, metric_name, trust_name, quarter_year) %>%
  ungroup()

# Repeats occuring when there is no alliance name
